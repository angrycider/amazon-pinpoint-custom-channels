# Phaxio Pinpoint Custom Channel
This SAM application will deploy all the resources necessary to build a Amazon Pinpoint Custom Channel to allow sending fax messages using [Phaxio](http://www.phaxio.com).

## Architecture
![architecture](architecture.png)

## Instructions
- You will need a [Phaxio](http://www.phaxio.com)  Phaxio provides free developer accounts with an initial budget of $1.00 and at $.07 per page this allows you to do enough for a POC.  However you can use the Test API credentials to simulate as many faxes as necessary.  You can also send a parameter to test failures.  When using the Test API Credentials, Phaxio will simulate the send and send a sample response.
- Get Phaxio API Key and Secret from [Phaxio Developer API Page](https://console.phaxio.com/api_credentials)
- Get Webhooktoken from [Phaxio Webhook Page](https://console.phaxio.com/user/callbacks/edit)
- Deploy SAM solution...the 3 items above are required parameters.  The SAM solution will also ask for an S3 Bucket name to create PDF files for faxing.
- The SAM solution will deploy and on the Outputs page you need to copy the PhaxioWebhookURL into the [Phaxio Webhook Page](https://console.phaxio.com/user/callbacks/edit)
...
## Output Parameters
- **PhaxioWebhookURL** - API Gateway endpoint URL. This is the URL to paste into [Phaxio Webhook Page](https://console.phaxio.com/user/callbacks/edit)
- **PinpointPhaxioCustomChannelFunction** - A function invoked by an Amazon Pinpoint Campaign or Journey that will send a fax via Phaxio
